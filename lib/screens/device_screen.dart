...
class _DeviceScreenState extends State<DeviceScreen> {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final readings = ReadingsDisplay(
          ...
          ch1Units: _continuityEnabled ? 'Ω' : 'A',
          ch1Max: _continuityEnabled ? 1000 : 10,
        ...
        final chart = Padding(
          ...
          child: RealtimeChart(
            ...
            ch1Label: 'CH1 · Current (A)',
            ch2Label: 'CH2 · Voltage (V)',
            style: _graphStyle,
          ),
        );
        return SafeArea(
  ...
  Widget _buildQuickControls(BleProvider provider) {
    return Material(
        ...
        child: Row(
          children: [
            FilterChip(
              ...
              label: const Text('CH1 · Current'),
              selected: _showCh1,
              onSelected: (value) => setState(() => _showCh1 = value),
            ),
            ...
            FilterChip(
              ...
              label: const Text('Continuity'),
              selected: _continuityEnabled,
              onSelected: (enabled) async {
                ...
                await provider.setContinuityEnabled(enabled);
              },
            ),
            const SizedBox(width: 6),
            FilterChip(
              avatar: const Icon(Icons.battery_charging_full, size: 16),
              label: const Text('CH2 · Voltage'),
              selected: _showCh2,
              onSelected: (value) => setState(() => _showCh2 = value),
            ),
  ...
  void _onBleProviderChanged() {
    ...
    if (_bleProvider.state != MmConnectionState.connected) {
      _sampleDebounce?.cancel();
      if (_ch1Points.isNotEmpty || _ch2Points.isNotEmpty) {
        setState(() {
          _ch1Points.clear();
          _ch2Points.clear();
          _pointCount = 0;
          ...
          children: [
            ...
            Wrap(
              ...
              children: ChannelMode.values.map((mode) {
                return ChoiceChip(
                  ...
                  selected: _ch1Mode == mode,
                  onSelected: (selected) {
                    if (selected) {
                      setState(() => _ch1Mode = mode);
                      if (mode.hardwareIndex >= 0) {
                        provider.setInputMode(0, mode.hardwareIndex);
                      }
                    }
                  },
                );
            ...
            Wrap(
              ...
              children: ChannelMode.values.map((mode) {
                return ChoiceChip(
                  label: Text(mode.label),
                  selected: _ch2Mode == mode,
                  onSelected: (selected) {
                    if (selected) {
                      setState(() => _ch2Mode = mode);
                      if (mode.hardwareIndex >= 0) {
                        provider.setInputMode(1, mode.hardwareIndex);
                      }
                    }
                  },
                );
            ...
            Wrap(
              spacing: 8,
              children: ['10A'].map((range) {
                final index = ['10A'].indexOf(range);
                return ChoiceChip(
                  label: Text(range),
                  selected: false,
                  onSelected: (selected) {
                    if (selected) provider.setRange(0, index);
                  },
                );
              }).toList(),
            ),
            ...
            const SizedBox(height: 8),
            const Text(
              'Auto uses the 60 V range for normal measurements and changes only when needed.',
            ),
            const SizedBox(height: 8),
            Wrap(
              spacing: 8,
              children: [
                ...
                for (final range in [(0, '60 V'), (1, '600 V')])
                  ChoiceChip(
                    label: Text(range.$2),
